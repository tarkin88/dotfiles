...
static const unsigned int gappfl         = 12;   /* gap between floating windows (when relevant) */
...
static int indicators[IndicatorLast] = {
	[IndicatorWs] = INDICATOR_TOP_BAR_SLIM,
...
static const char *const autostart[] = {
  "bash",
  "-c",
  "'/home/frank/.local/bin/autostart.sh'",
	NULL /* terminate */
};
...
static const Rule clientrules[] = {
	...
	{ .instance = "spterm (w)", .scratchkey = 'w', .floatpos = "50% 10% 90% 65%", .flags = Floating },
	{ .instance = "spterm (e)", .scratchkey = 'e', .floatpos = "50% 99% 80% 45%", .flags = Floating },
	{ .instance = "spfm (r)", .scratchkey = 'r', .floatpos = "50% 50% 40% 25%", .flags = Floating },
  { .instance = "spterm (`)", .scratchkey = '`', .flags = AttachMaster },
	{ .instance = "spbt (t)", .scratchkey = 't', .floatpos = "50% 0% 40% 45%", .flags = Floating },
  { .instance = "spvol (v)", .scratchkey = 'v',  .floatpos = "50% 100% 40% 25%", .flags = Floating },
	{ .title = "Huddle", .flags = Floating|Centered },
...
static const BarRule barrules[] = {
	...
	{  0,       0,     5,       5,   5,   0,     BAR_ALIGN_RIGHT,        size_systray,             draw_systray,            click_systray,            NULL,                     "systray" },

	{ -1,       0,     6,       5,   5,  	0,     BAR_ALIGN_RIGHT,        size_ltsymbol,            draw_ltsymbol,           click_ltsymbol,           NULL,                     "layout" },
	...
	{  0,       0,     0,       0,   0,   0,     BAR_ALIGN_CENTER,       size_status,              draw_status,             click_status,             NULL,                     "status0" },
	...
	// { -1,       0,     0,       0,   0,   0,     BAR_ALIGN_RIGHT_RIGHT,  size_wintitle_hidden,     draw_wintitle_hidden,    click_wintitle_hidden,    NULL,                     "wintitle_hidden" },
	// { -1,       0,     0,       0,   0,   0,     BAR_ALIGN_LEFT_LEFT,    size_wintitle_floating,   draw_wintitle_floating,  click_wintitle_floating,  NULL,                     "wintitle_floating" },
...
static const char *spcmd_s[] = {"s", NULL };
static const char *spcmd_w[] = {"w", "st", "-n", "spterm (w)", NULL };
static const char *spcmd_e[] = {"e", "st", "-n", "spterm (e)", NULL };
static const char *spcmd_extra[] = {"`", "st", "-n", "spterm (`)", NULL };
static const char *spcmd_t[] = {"t", "st", "-n", "spbt (t)", "-e", "bluetui",  NULL };
static const char *spcmd_r[] = {"r", "st", "-n", "spfm (r)", "-e", "ranger", NULL };
static const char *spcmd_v[] = {"v", "st", "-n", "spvol (v)", "-e", "pulsemixer", NULL };

static const char *statusclickcmd[] = { "~/.local/bin/statusbar-click.sh", NULL };
static const char *clipboardmenu[] = { "~/.local/bin/clipboard-menu", NULL };
static const char *appsmenu[] = { "~/.local/bin/favorites-apps-menu", NULL };
static const char *screenshotmenu[] = { "~/.local/bin/screenshots-menu", NULL };
static const char *powermenu[] = { "~/.local/bin/power-menu", NULL };
static const char *keyboardmenu[] = { "~/.local/bin/keyboard-swithcer-menu", NULL };
...
static const char *lightup[]   = { "/sbin/brightnessctl", "s", "+10%", NULL };
static const char *lightdown[] = { "/sbin/brightnessctl", "s", "10%-", NULL };
...
static Key keys[] = {
	...
	{ KeyPress,   Ctrl|Alt,                     XK_Return,       riospawn,               {.v = termcmd } }, // draw/spawn a terminal
	{ KeyPress,   MODKEY|Shift,                 XK_b,            togglebar,              {0} }, // toggles the display of the bar(s) on the current monitor
  { KeyPress,   ControlMask|Alt,              XK_p,            spawn,                  {.v = appsmenu }},
	{ KeyPress,   ControlMask|Alt,              XK_h,            spawn,                  {.v = clipboardmenu }},
	{ KeyPress,   ControlMask|Alt,              XK_x,            spawn,                  {.v = powermenu }},
	{ KeyPress,   ControlMask|Alt,              XK_space,        spawn,                  {.v = keyboardmenu }},
	{ KeyPress,   0,                            XK_Print,        spawn,                  {.v = screenshotmenu }},
  ...
  { KeyPress,   0,                            XF86XK_MonBrightnessUp,                  spawn,                  {.v = lightup } },
	{ KeyPress,   0,                            XF86XK_MonBrightnessDown,                spawn,                  {.v = lightdown } },
...
//	STACKKEYS(AltGr|Shift,                                       stackswap)                            // swap the currently focused client with the nth client in the stack
  ...
  SCRATCHKEYS(MODKEY,                         XK_t,            spcmd_t)
	...
	SCRATCHKEYS(MODKEY,                         XK_grave,        spcmd_extra)
